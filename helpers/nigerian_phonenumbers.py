"""
This implementation helps us identify a nigerian phone number
Date: 01-November-2022
Adopted/Modified By: Olaoluwa Faniyi @gofaniyi
Main Author: https://github.com/djunehor
Source: https://github.com/djunehor/python_validate_nigerian_phone
"""
import re
...
class NigerianPhone:
    ...
    def is_valid(self):
        ...
        # doesn't contain country calling code
            ...
            if (
                self.line[:1] == "0"
                and len(self.line) == 11
                and (self.line[:5] in self.prefixes or self.line[:4] in self.prefixes)
            ):
                self.is_valid_phone = True
                return True

            # check if it starts with any prefix without 0 e.g [807,906, 701 etc..]
            elif len(self.line) == 10 and (
                self.line[:4] in self.prefixes or self.line[:3] in self.prefixes
            ):
            ...
            # check if it's a land line starting with 0
                ...
                self.line[:1] == 0
                and self.line[:3].replace(self.line[:1], "") in self.area_codes
                and len(self.line) == 8
            ):
                self.is_valid_land_line = True
                return True
    ...
    def is_mtn(self):
        if (
            self.formatted()[:5] in prefixes["visafone"]
            or self.formatted()[:4] in prefixes["mtn"]
        ):
            return True
        return False
    ...
    def is_multilinks(self):
        if (
            self.formatted()[:5] in prefixes["multilinks"]
            or self.formatted()[:4] in prefixes["multilinks"]
        ):
            return True
        return False
    ...
    def is_visafone(self):
        if (
            self.formatted()[:5] in prefixes["visafone"]
            or self.formatted()[:4] in prefixes["visafone"]
        ):
            return True
        return False
    ...
    def is_starcomms(self):
        if (
            self.formatted()[:5] in prefixes["starcomms"]
            or self.formatted()[:4] in prefixes["starcomms"]
        ):
            return True
        return False
